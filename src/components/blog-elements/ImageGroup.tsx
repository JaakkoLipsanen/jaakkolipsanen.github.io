...
const generateRows = (images: Image[]): ImageRow[] => {
	...
	const getNextRowImageCount = () => {
		...
		else { return isPortrait(currentImage) ? 2 : 1; }
	};
	...
	return (
		<GroupRowContainer>
			{props.images.map(img => 
				<GroupImage 
					key={img.filename}
					src={aws.getImageUrl(props.blogPostName, ImageQuality.FullHD, img.filename)}
				/> 
			)}
		</GroupRowContainer>
	);
...
const ImageGroup = (props: ImageGroupProps) => {
	...
	return (
		<ImageGroupContainer>
			{rows.map((row, i) => <GroupRow key={i} images={row.images} blogPostName={props.blogPostName} />)}
		</ImageGroupContainer>
	);
};
